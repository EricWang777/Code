...
string Player::stringTiles(){
    string reString = "";
    for (set<Tile*>::iterator it = _tiles.begin();
         it != _tiles.end(); ++it) {
         stringstream ss;
         ss << (*it)->getPoints();
        reString = reString + string(1, (*it)->getLetter()) + "(" + ss.str() + ")" + " ";
    }
    return reString;
}
...
bool Player::exchange(string word, Bag &bag){
    ...
    set<Tile*> temp;
    for (unsigned int i = 0; i < noAfterQ_word.length(); i++) {
...
bool Player::placeHorizontal(int row, int col, string word, Board &board, Dictionary &dict){
    ...
    preRow.clear();
    takenTiles.clear();
    for (unsigned int i = 0; i < noAfterQ_word.size(); i++) {
        ...
        if (test == "") { // find a non-exist word
            ...
            cout << " on (" << row+1 << ", " << col+1 << ")\n";
            // reset the board
            for (unsigned int i = 0; i < preRow.size(); i++) {
...
bool Player::placeVertical(int row, int col, string word, Board &board, Dictionary &dict){
    ...
    for (unsigned int i = 0; i < noQ_word.size(); i++) {
        while (board.emptySlot(row, col) == false) {
            row++;
        }
        board.modify(row, col, noQ_word[i]);
