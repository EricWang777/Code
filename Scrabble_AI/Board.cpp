...
// initalize the baord
// initBoard()
void Board::iniBoard(std::string init_file){
    if (init_file == "")
    {
        return;
    }
    cout << "initalizing Board...\n";
    ifstream boardFile (init_file.c_str());
    string row;
    if (!boardFile.is_open())
        throw invalid_argument("Cannot open file: " + init_file);
    for (int i = 0 ; i < _x; ++ i)
    {
        getline (boardFile, row);
...
bool Board::emptySlot (int row, int col){
    if (row < 0 || col < 0) {
       // cout << "out of min bound" << endl;
        return false;
    }
    if (row >= _x || col >= _y) {
       // cout << "out of max bound" << endl;
        return false;
    }
...
string Board::word_row(int row, int col, int &score, Dictionary &dict){
    ...
    int headCol = col;
    while (col >= 0 && col < _x && !emptySlot(row, col)) {
    ...
    if (reval.length() > 1) {
        ...
        col = headCol;
        while (col >= 0 && col < _y && !emptySlot(row, col)) {
...
string Board::word_col(int row, int col, int &score, Dictionary &dict){
    ...
    int headRow = row;
    while (row >= 0 && row < _x && !emptySlot(row, col)) {
        reval += _board[row][col];
