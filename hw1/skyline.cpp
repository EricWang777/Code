...
int main(int argc, char* argv[])
{
  ...
  while(getline(input, curr)) {
	  ...
	  if (curr == "BUILD") {
		  ...
		  ss >> y;
		  ss >> c;
		  if (ss.fail() || skyline[x-1] != NULL) {
			  output << "Error - incorrect command" << endl;
		  }
		  else {
		  	  Build(skyline[x-1], y, x, c, buildingheights[x-1], output);
		  }
	  }
	  else if (curr == "EXTEND") {
	  	int x, y;
	  	...
	  	if (ss.fail() || skyline[x-1] == NULL){
	  		...
	  		Extend(skyline[x-1], y, x, c, buildingheights[x-1], output);
	  	}
	  	...
	  	if (ss.fail() || skyline[x-1] == NULL){
	  		...
	  		Demolish(skyline[x-1], output, x, buildingheights[x-1]);
	  	}
	  }
